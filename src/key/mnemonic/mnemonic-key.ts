...
const WORDS_LENGTH_STRENGTH_MAP = new Map<number, number>([
  ...
  export class MnemonicKey implements IKeyManager {
    ...
    generateWithWords(words:string):Uint8Array{
      if(!words){
        throw new Error(`Mnemonic may be null`)
      }
      const wordArray = words.split(" ");
      if(!WORDS_LENGTH_STRENGTH_MAP.has(wordArray.length)){
        throw new Error(`Length of words must be in allowed list: ${Array.from(WORDS_LENGTH_STRENGTH_MAP.keys()).join(", ")}`)
      }
      //stringArray 转化为 Uint8Array
       
      return this.getWordListCoder().decode(wordArray);
    ...
    toKeyToString(entropy: Hex, encode = false): string {
      const entropyArr = TypeUtils.parseHexToArray(entropy);
      assertEntropy(entropyArr);
      const words = this.getWordListCoder().encode(entropyArr);
      if (encode) {
        ...
        // Clear the sensitive value
        TypeUtils.clearArray(words);
        return encodedWords.toString().replace(/,/g," ");
      }
    ...
    mnemonicToSeedSync(
      ...
      const mnemonicBuffer = Uint8Array.from(
        Buffer.from(this.normalize(mnemonic), 'utf8'),
      );
      const saltBuffer = Uint8Array.from(
        Buffer.from(this.salt(this.normalize(password)), 'utf8'),
      );
    ...
    private salt(password?: string): string {
